...
export async function exportarPrenomina(mes) { const response = await api.get(`/laboral/${mes}/export-prenomina`, { responseType: 'blob' }); const url=URL.createObjectURL(response.data); const link=document.createElement('a'); link.href=url; link.download=`prenomina-${mes}.csv`; link.click(); URL.revokeObjectURL(url); }
