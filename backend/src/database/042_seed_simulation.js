module.exports = async function (client) {
  console.log('--- STARTING ESSART S.A. MOCK DATA SEEDING (MIGRATION) ---');
  ...
  const insertMarcacion = async (email, fecha, horaEntrada, horaSalida, options = {}) => {
    ...
    const insert = async (tipo, hora, novelty) => {
      if (!hora) return;
      const detail = novelty || (tipo === 'entrada' && isReplacement ? `Reemplazo autorizado en ${branchCode}` : null);
      await client.query(`
      ...
      `, [
        ...
        novelty ? (tipo === 'entrada' ? 'atraso' : 'salida_novedad') : isReplacement && tipo === 'entrada' ? 'reemplazo_sucursal' : null,
        detail,
      ]);
    };
  ...
  console.log('--- MOCK DATA SEEDING COMPLETED SUCCESSFULLY (MIGRATION) ---');
};
