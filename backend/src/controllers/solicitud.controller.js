...
async function review(req, res, next) { try { res.json({ ok: true, data: await service.reviewSolicitud({ empresaId: empresaId(req), solicitudId: req.params.id, reviewerId: req.auth.usuario_id, auth: req.auth, decision: req.body.decision, comentario: req.body.comentario, datos_adicionales: req.body.datos_adicionales, reemplazo_empleado_id: req.body.reemplazo_empleado_id }) }); } catch (error) { next(error); } }
